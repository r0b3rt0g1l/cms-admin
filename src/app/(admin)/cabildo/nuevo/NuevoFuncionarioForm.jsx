...
import {
  TIPOS_CABILDO,
  TIPOS_CABILDO_ELECTOS,
  TIPOS_DIRECTORIO_OTROS,
} from "@/lib/cabildo-constants";

export default function NuevoFuncionarioForm({ grupo = null }) {
  const [state, formAction, pending] = useActionState(createFuncionarioAction, {});
  const [preview, setPreview] = useState(null);
  const [fileName, setFileName] = useState("");

  // Acota el dropdown de Tipo según la sección de origen (sin preseleccionar).
  const setTipos =
    grupo === "cabildo"
      ? TIPOS_CABILDO_ELECTOS
      : grupo === "directorio"
        ? TIPOS_DIRECTORIO_OTROS
        : null;
  const tiposDisponibles = setTipos
    ? TIPOS_CABILDO.filter((t) => setTipos.includes(t.value))
    : TIPOS_CABILDO;

  const volverHref = grupo === "directorio" ? "/directorio" : "/cabildo";
  const volverLabel =
    grupo === "directorio" ? "← Volver al directorio" : "← Volver al cabildo";
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        <Link href={volverHref} className="text-sm text-gray-500 hover:text-gray-800">
          {volverLabel}
        </Link>
        <h1 className="text-3xl font-bold mt-2">Nueva persona</h1>
        <p className="text-gray-600 mt-1">
          Elige el <strong>Tipo</strong> (define en qué sección del sitio
          aparece) y escribe el cargo tal cual se verá. La foto es opcional y la
          puedes subir después.
        </p>
      </div>
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Recomendado: foto cuadrada de buena calidad, mínimo 400×400 px. Si no
            la subes ahora, podrás agregarla después editando la persona.
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            maxLength={500}
            placeholder="Breve presentación de la persona"
            className="w-full border border-gray-300 rounded-md px-3 py-2"
          />
        </div>
