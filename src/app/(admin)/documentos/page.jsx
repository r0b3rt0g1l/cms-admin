...
import { listDocumentosAction } from "@/lib/actions";
...
export default async function DocumentosPage() {
  const result = await listDocumentosAction();
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Sube y administra documentos del municipio.
          </p>
        </div>
        <Link
      ...
      {documentos.length === 0 && !error ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          <p className="text-gray-500">
            No hay documentos todavía. Sube el primero para empezar.
          </p>
        </div>
      ) : (
                ...
                {documentos.map((doc) => (
                  <tr key={doc.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      ...
                      <DeleteDocumentoButton id={doc.id} titulo={doc.titulo} />
                    </td>
                  </tr>
                ))}
              </tbody>
